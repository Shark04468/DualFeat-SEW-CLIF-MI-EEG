...
class _ForwardConstrainedLinear(torch.nn.Module):
    ...
    def forward(self, value: torch.Tensor) -> torch.Tensor:
        self.linear.weight.data = torch.renorm(self.linear.weight.data, p=2, dim=0, maxnorm=0.25)
...
def test_official_source_locks_require_exact_commit_and_archive(tmp_path: Path) -> None:
    for name, (commit, archive) in SOURCE_LOCKS.items():
        ...
        directory.mkdir()
        (directory / "SOURCE_LOCK.txt").write_text(f"{name} {commit} {archive}\n", encoding="utf-8")
...
def test_evaluation_state_canonicalization_handles_forward_constraints() -> None:
    model = _ForwardConstrainedLinear()
    reference = np.ones((2, 4), dtype=np.float32)
    assert canonicalize_evaluation_state("forward_constrained", model, reference, device="cpu")
    canonical = {key: value.detach().clone() for key, value in model.state_dict().items()}
    model(torch.from_numpy(reference[:1]))
    assert all(torch.equal(value, model.state_dict()[key]) for key, value in canonical.items())
