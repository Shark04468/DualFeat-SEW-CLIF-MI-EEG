...
def task_carrier(
    ...
    *,
    ...
    expected = round((task_stop - task_start) * sfreq)
    if int(baseline.sum()) != round(sfreq) or int(task.sum()) != expected:
        raise ValueError(
...
def fbcnet_filterbank(x: np.ndarray, sfreq: float = 250.0) -> np.ndarray:
    ...
    for index, (low, high) in enumerate(FBCNET_BANDS):
        ...
        output[..., index] = signal.lfilter(numerator, denominator, array, axis=-1).astype(
            np.float32
        )
...
def prepare_model_input(name: str, x: np.ndarray, sfreq: float = 250.0) -> np.ndarray:
    ...
    if name == "eegnet":
        ...
        if resampled.shape[-1] != 256:
            raise RuntimeError(
                f"EEGNet official view must contain 256 samples, got {resampled.shape}"
            )
...
def canonicalize_evaluation_state(
    ...
    *,
    ...
    """Apply and verify idempotent official forward-time constraints before sealing."""

    reference = np.asarray(x_reference, dtype=np.float32)
    if reference.shape[0] == 0:
        raise ValueError(f"{name} state canonicalization requires a training example")
    model.eval()
    ...
    canonical = {key: value.detach().cpu().clone() for key, value in model.state_dict().items()}
    _logits(model, torch.from_numpy(reference[:1]).to(device))
    repeated = model.state_dict()
    changed_again = [
        key
        for key, value in canonical.items()
        if not torch.equal(value, repeated[key].detach().cpu())
    ]
    if changed_again:
        raise RuntimeError(
            f"{name} forward-time state constraint is not idempotent: {changed_again}"
        )
    return any(not torch.equal(before[key], value) for key, value in canonical.items())
...
def fit_baseline(
    ...
    *,
    ...
    batch_size = int(settings["batch_size"] if physical_batch_size is None else physical_batch_size)
    effective_batch = int(batch_size if effective_batch_size is None else effective_batch_size)
    ...
    if effective_batch < batch_size or effective_batch % batch_size != 0:
        raise ValueError("effective batch size must be an integer multiple of physical batch size")
