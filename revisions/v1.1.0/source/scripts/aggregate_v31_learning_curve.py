...
def _cluster_bootstrap(
    ...
    for row in rows:
        by_dataset.setdefault(str(row["dataset"]), {}).setdefault(int(row["subject"]), []).append(
            row
        )
    values: list[float] = []
...
def main() -> None:
    ...
    keys = sorted(
        {(row["dataset"], row["subject"], row["seed"], row["budget"]) for row in metric_rows}
    )
    ...
    subject_keys = sorted({(row["dataset"], row["subject"], row["budget"]) for row in paired})
    for dataset, subject, budget in subject_keys:
        values = [
            row
            for row in paired
            if (row["dataset"], row["subject"], row["budget"]) == (dataset, subject, budget)
        ]
        subject_rows.append(
            {
                "dataset": dataset,
                "subject": subject,
                "budget": budget,
                "examples_per_class": float(np.mean([row["examples_per_class"] for row in values])),
    ...
    if config.get("primary_analysis", {}).get("expected_direction") == "none_for_smoke":
        decision = {
            "schema": "dpc-snn-v31-learning-curve-decision/v1",
            "status": "engineering_canary_no_inference",
            "slope_accuracy_gain_per_log_example": None,
            "bootstrap_95_ci": None,
            "bootstrap_repetitions": 0,
            "dataset_slopes": {},
            "interpretation_scope": "pipeline_validation_only",
            "not_for_scientific_inference": True,
        }
    ...
    decision = {
        ...
        "one_sided_bootstrap_p": float(
            (np.count_nonzero(bootstrap >= 0.0) + 1) / (repetitions + 1)
        ),
