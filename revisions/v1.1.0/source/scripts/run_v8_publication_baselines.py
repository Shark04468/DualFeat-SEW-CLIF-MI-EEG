...
from dpc_snn.analysis.v8_utility import multiclass_calibration_metrics
...
from dpc_snn.data.openbmi import load_openbmi_subject
...
def _openbmi_view(
    *,
    ...
    cache_root = default_openbmi_v8_cache_root()
    cache_disabled = os.environ.get("DPC_SNN_OPENBMI_V8_CACHE_DISABLE", "").lower() in {
        "1",
        "true",
        "yes",
    }
    if cache_root is not None and not cache_disabled:
        npz_path, json_path = openbmi_v8_cache_paths(cache_root, subject, session)
        if npz_path.is_file() or json_path.is_file():
            return load_openbmi_v8_cache(
                cache_root,
                subject=subject,
                session=session,
                role="evaluation" if role == "evaluation" else "training",
...
def _train_subject(
    ...
    prepared = {
        model: prepare_model_input(model, normalized, sfreq=sfreq) for model in scope["models"]
    }
...
def _build_barrier(output: Path, contract: dict[str, Any], scope: dict[str, Any]) -> dict[str, Any]:
    records: list[dict[str, Any]] = []
...
def _evaluate_subject(
    ...
    for model_name in scope["models"]:
        ...
        region_inputs = {
            name: prepare_model_input(
                model_name,
                zero_reference_region(normalized, channel_names=channel_names, region=name),
                sfreq=sfreq,
            )
                ...
                rows.append(
                    {
                        ...
                        "negative_log_likelihood": metrics["calibration"][
        ...
        summaries.append(
            {
                ...
                "subject_accuracy_std": float(np.std(subject_values, ddof=1))
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--phase",
        choices=("init", "train", "seal", "evaluate", "aggregate", "all"),
        default="all",
    )
    ...
    parser.add_argument(
        "--storage-root", default=os.environ.get("DPC_SNN_STORAGE_ROOT", "artifacts/storage")
    )
...
def main() -> None:
    ...
    if args.phase == "init":
        ...
        write_json(
            ...
            {
                "status": "initialized_before_training",
                "dataset": args.dataset,
                "contract_sha256": contract["combined_sha256"],
            },
        )
    ...
    if args.phase == "aggregate":
        ...
        write_json(
            output / "campaign_status.json",
            {
                ...
                "expected_runs": len(scope["subjects"])
                * len(scope["models"])
                * len(scope["seeds"]),
                "posthoc_explanatory": True,
            },
