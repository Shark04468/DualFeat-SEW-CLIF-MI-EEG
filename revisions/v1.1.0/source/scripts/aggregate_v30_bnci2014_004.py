...
def _evaluation_trial_bounds(
    ...
    if (
        erratum.get("schema") != "dpc-snn-v30-post-barrier-metadata-erratum/v1"
...
def main() -> None:
    ...
    expected = len(config["dataset"]["subjects"]) * len(config["seeds"]) * len(config["variants"])
    ...
    indices = rng.integers(0, gain_array.size, size=(int(args.bootstrap_samples), gain_array.size))
    bootstrap = gain_array[indices].mean(axis=1)
    recovery = config.get("status") == "recovered_parent_replication"
    if recovery:
        decision = {
            "status": "retrospective_recovery_summary",
            "promotion_gate_applied": False,
            "mean_snn_minus_ann_pp": float(gain_array.mean()),
            "positive_subjects": int(np.count_nonzero(gain_array > 0.0)),
            "subjects": int(gain_array.size),
            "one_sided_wilcoxon_p_descriptive_only": p_value,
        }
    else:
    ...
    decision.update(
        {
            ...
            "atcnet_grand_accuracy": float(
                np.mean([row["accuracy"] for row in reference_rows if row["variant"] == "atcnet"])
            ),
            "fbcnet_grand_accuracy": float(
                np.mean([row["accuracy"] for row in reference_rows if row["variant"] == "fbcnet"])
            ),
            "equal_teacher_grand_accuracy": float(
                np.mean(
                    [row["accuracy"] for row in reference_rows if row["variant"] == "equal_teacher"]
                )
            ),
            "inferential_unit": "subject_after_averaging_paired_seeds",
            "seeds_per_subject": len(config["seeds"]),
            "project_level_blind_confirmation": not recovery,
            "retrospective_recovery_not_blind": recovery,
        }
    )
    ...
    if not recovery and decision["status"] != "pass":
        raise SystemExit(2)
