...
from dpc_snn.data.bnci2014_004 import load_bnci2014_004_subject
...
def _teacher_path(output: Path, subject: int, seed: int, model: str) -> Path:
    return output / f"subject_{subject:02d}" / f"seed_{seed}" / "teachers" / model / "checkpoint.pt"


def _student_path(output: Path, subject: int, seed: int, variant: str) -> Path:
    return (
        output / f"subject_{subject:02d}" / f"seed_{seed}" / "students" / variant / "checkpoint.pt"
    )
...
def _data_view(
    ...
    carrier = task_carrier(x, sfreq=float(data["sfreq"]), epoch_tmin=float(data["epoch_tmin"]))
...
def _prepared_inputs(carrier: np.ndarray, gain: FixedGain, sfreq: float) -> dict[str, np.ndarray]:
    ...
    return {
        model: prepare_model_input(model, normalized, sfreq=sfreq) for model in ("atcnet", "fbcnet")
    }
...
def _train(args: argparse.Namespace, config: dict[str, Any], freeze: dict[str, Any]) -> None:
    ...
    for seed in config["seeds"]:
        ...
        fingerprint = _training_fingerprint(freeze, config, data_identity, args.subject, int(seed))
        ...
        for model_index, model_name in enumerate(config["teacher_models"]):
            checkpoint_path = _teacher_path(output, args.subject, int(seed), str(model_name))
            ...
            if checkpoint_path.is_file() and metrics_path.is_file():
                ...
                run_seed = (
                    3_000_000 + args.subject * 10_000 + int(seed) * 101 + model_index * 1_000_000
                )
                fit = fit_baseline(
                    str(model_name),
                    source_root=Path(args.source_root).resolve(),
                    ...
                    run_label=(f"V30:S{args.subject}:seed{seed}:teacher:{model_name}"),
                    n_channels=len(config["dataset"]["channels"]),
                    n_classes=2,
                    physical_batch_size=int(
                        config["training"]["teacher_physical_batch_size"][str(model_name)]
                    ),
                ...
                write_json(
                    metrics_path,
                    {
                        ...
                        "subject": args.subject,
                        "seed": int(seed),
                        "fixed_epochs": int(config["training"]["teacher_fixed_epochs"]),
        ...
        for variant in config["variants"]:
            checkpoint_path = _student_path(output, args.subject, int(seed), str(variant))
            metrics_path = checkpoint_path.with_name("training_metrics.json")
            if checkpoint_path.is_file() and metrics_path.is_file():
                checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
                if (
...
def _metric_payload(labels: np.ndarray, logits: np.ndarray) -> dict[str, float]:
    ...
    return {
        key: float(metrics[key]) for key in ("accuracy", "balanced_accuracy", "kappa", "macro_f1")
    }
...
def _evaluate(args: argparse.Namespace, config: dict[str, Any], freeze: dict[str, Any]) -> None:
    ...
    for seed in config["seeds"]:
        ...
        eval_atc, eval_fbc = standardizer.transform(features["atcnet"], features["fbcnet"])
        for variant in config["variants"]:
            ...
            student_dir = ensure_dir(seed_dir / "students" / str(variant) / "evaluation")
...
def main() -> None:
    ...
    if freeze.get("status") not in {
        "frozen_before_any_bnci2014_004_evaluation_access",
        "frozen_before_recovery_run_evaluation_access",
    }:
