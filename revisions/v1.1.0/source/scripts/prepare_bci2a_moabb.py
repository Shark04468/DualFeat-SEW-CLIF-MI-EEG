...
def main() -> None:
    ...
    expected_samples = round((args.tmax - args.tmin) * args.resample)
    if x.shape[-1] not in {expected_samples, expected_samples + 1}:
        raise RuntimeError(
            f"BCI2a epoch has {x.shape[-1]} samples; expected {expected_samples} "
            f"or {expected_samples + 1} before half-open cropping"
        )
    ...
    if len(ch_names) != x.shape[1]:
        raise ValueError(
            f"MOABB returned {x.shape[1]} channels but raw metadata contains {len(ch_names)} EEG channels"
        )
    ...
    for subject in sorted(set(subject_arr)):
        ...
        out = (
            output / f"A{int(subject):02d}.npz"
            if str(subject).isdigit()
            else output / f"{subject}.npz"
        )
        save_npz(
            out,
            ...
            label_names=np.asarray(
                [k for k, _ in sorted(labels.items(), key=lambda item: item[1])]
            ),
