...
def main() -> None:
    ...
    teacher_counts: dict[str, set[int]] = {
        model: set() for model in config["teacher_models"]
    }
    student_counts: dict[str, set[int]] = {
        variant: set() for variant in config["variants"]
    }
    ...
    payload = {
        ...
        "teacher_parameter_counts": {
            key: sorted(value) for key, value in teacher_counts.items()
        },
        "student_parameter_counts": {
            key: sorted(value) for key, value in student_counts.items()
        },
