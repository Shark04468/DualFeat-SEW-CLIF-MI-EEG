...
from dpc_snn.experiments.v31_learning_curve import (  # noqa: E402
    LearningCurveSubset,
...
def _publication_checkpoint(
    args: argparse.Namespace, model: str, subject: int, seed: int
) -> Path:
    return (
...
def _load_view(
    ...
    data, labels, trial_ids, identity = _bnci_view(
        v30_config, args.subject, sessions
    )
...
def _feature_cache(
    ...
    teacher_paths = {
        name: _teacher_checkpoint(args, name, args.subject, seed)
        for name in ("atcnet", "fbcnet")
    }
    fingerprint = {
...
def _train(
    ...
    prepared, labels, _, data_identity = _load_view(args, config, "training")
    ...
    for seed in config["seeds"]:
        ...
        for subset in _subsets(config, labels, args.dataset, args.subject, seed):
            ...
            if subset_metadata_path.is_file() and read_json(subset_metadata_path) != subset_metadata:
            ...
            if standardizer_path.is_file():
                ...
                standardizer = fit_feature_standardizer(
                    atc[subset.indices], fbc[subset.indices]
                )
            ...
            for variant in config["variants"]:
                ...
                fit = fit_v9_dual_feature(
                    ...
                    run_label=(
                        f"V31:{args.dataset}:S{args.subject}:seed{seed}:"
                        f"{subset.label}:{variant}"
                    ),
                )
                _atomic_torch_save(
                    {
...
def _evaluate(
    ...
    for seed in config["seeds"]:
        ...
        training_subset_root = output / args.dataset / f"subject_{args.subject:02d}" / f"seed_{seed}"
